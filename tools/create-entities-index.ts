...
function createEntitiesIndex() {
  console.log('Creating entity-index.ts for mmt-api app');
  const src = `${path.dirname(__dirname)}/apps/mmt-api/src`;
  if (!fs.existsSync(src)) {
    console.log(`App mmt-api cannot be found. Path not exist: ${src}`);
    process.exit(1);
  }
